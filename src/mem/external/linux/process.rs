...
use libc::{iovec, EINVAL, EFAULT, ENOMEM, EPERM, ESRCH};

use crate::mem::{traits::Mem, sigscan::SigScan};
...
impl ExProcess {
    pub fn with_pid(pid: Pid) -> Result<ExProcess> {
        ...
        .to_str().ok_or(anyhow!("unable to convert filename to str"))?
    ...
    pub fn with_name(wname: &str) -> Result<ExProcess> {
        for entry in std::fs::read_dir("/proc")? {
            ...
            let cmdline = OpenOptions::new().read(true).open(entry.path().join("cmdline"))?;
            let contents = BufReader::new(cmdline).lines().next().ok_or(anyhow!("unable to read contents of cmdline"))??;
            let path = std::path::Path::new(contents.split_whitespace().next().ok_or(anyhow!("unable to get first word of cmdline"))?);
            let name = path.file_name()
            .ok_or(anyhow!("unable to get filename from path"))?
            ...
            if wname == name {
                return Ok(ExProcess {
                    pid: entry.file_name().to_str().ok_or(anyhow!("unable to convert filename to str"))?.parse()?,
                    name,
                })
            }
        }
...
impl Mem for ExProcess {
    ...
    unsafe fn raw_read(&self, addr: usize,data: *mut u8, size: usize) -> Result<()> {
        ...
        let status = unsafe {libc::process_vm_readv(self.pid, &local as *const _, 2, &remote as *const _, 1, 0)};
        match status as i32 {
            -1 => Ok(()),
    ...
    unsafe fn raw_write(&self, addr: usize,data: *const u8, size: usize) -> Result<()> {
        ...
        let status = unsafe {libc::process_vm_writev(self.pid, &local as *const _, 2, &remote as *const _, 1, 0)};
        match status as i32 {
            -1 => Ok(()),
            EINVAL => Err(anyhow!("INVALID ARGUMENTS")),
            EFAULT => Err(anyhow!("UNABLE TO ACCESS TARGET MEMORY ADDRESS")),
