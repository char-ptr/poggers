...
impl<'a> Process {
    ...
    fn get_pid_from_name(proc_name :&str) -> Result<u32> {
        ...
        let hsnapshot = unsafe {CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)}.or(Err(ProcessError::UnableToOpenProcess(StringOru32::String(proc_name.to_string()))))?;
        ...
        if ! hres.as_bool() {
            return Err(ProcessError::UnableToOpenProcess(StringOru32::String(proc_name.to_string())).into());
        }
        if le_poggier(&pe) {
            result.replace(pe.th32ProcessID);
        }
        while unsafe {Process32Next(hsnapshot, &mut pe)}.as_bool() && result.is_none() && pe.th32ProcessID != 0 {
            if le_poggier(&pe) {
                result.replace(pe.th32ProcessID);
                break;
            } else {
        ...
        result.ok_or(ProcessError::NoProcessFound(StringOru32::String(proc_name.to_string())).into())
    ...
    fn get_name_from_pid(process_id:u32) -> Result<String> {
        ...
        let hndl = unsafe {OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, false, process_id).or(Err(ProcessError::UnableToOpenProcess(StringOru32::U32(process_id))))?};
        if hndl.is_invalid() {
        ...
        // println!("{:?}", buffer);
        Ok(String::from_utf16_lossy(&buffer).rsplit("\\").next().unwrap().trim_matches('\x00').to_string())
    }

    fn open_handle(process_id: u32) -> Result<HANDLE> {
        let hndl = unsafe {OpenProcess(PROCESS_ALL_ACCESS, false, process_id).or(Err(ProcessError::UnableToOpenProcess(StringOru32::U32(process_id))))?};
        if hndl.is_invalid() {
    ...
    pub fn read<T : Default>(&self, addr:usize) -> Result<T> {
        ...
        let res = unsafe { Toolhelp32ReadProcessMemory(self.pid, 
    ...
    pub fn write<T>(&self, addr: usize, data: &T) -> Result<()> {
        ...
        let d = unsafe {
            ...
            WriteProcessMemory(self.handl, addr as *const c_void, d , std::mem::size_of::<T>(), &mut 0 )
        };
    ...
    pub fn change_protection(&self, addr: usize, size: usize, protection: PAGE_PROTECTION_FLAGS) -> Result<PAGE_PROTECTION_FLAGS> {
        let mut old_protect = Default::default();
        let res = unsafe { VirtualProtectEx(self.handl, addr as *const c_void, size, protection, &mut old_protect) };
        if res.as_bool() {
            Ok(old_protect)
        } else {
