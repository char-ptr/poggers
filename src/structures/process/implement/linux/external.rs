...
impl Mem for Process<External> {
    ...
    #[inline]
    unsafe fn alter_protection(&self,_addr:usize, _size: usize, _prot: Protections) -> Result<Protections,crate::traits::MemError> {
    ...
    /// will always return unsupported.
    #[inline]
    unsafe fn raw_virtual_alloc(&self, _addr:Option<usize>, _size:usize, _prot: Protections) -> Result<usize,crate::traits::MemError> {
    ...
    /// will always return unsupported.
    #[inline]
    unsafe fn raw_virtual_free(&self, _addr:usize, _size:usize) -> Result<(),crate::traits::MemError> {
        Err(crate::traits::MemError::Unsupported)
    }
}
impl Process<External> {
    ...
    pub fn find_by_name(name: &str) -> Result<Self, crate::structures::process::ProcessError> {
        let dir = std::fs::read_dir("/proc/").map_err(|_| ProcessError::UnableToFindProcess(U32OrString::String(name.to_string())))?;
        for file in dir {
            let Ok(file) = file else {
                continue;
            };
            if file.file_name().to_string_lossy().chars().any(|x|x.is_alphabetic()) {
                continue;
            }
            let path = file.path().join("comm");
            let f = std::fs::read_to_string(path).map_err(|_| ProcessError::UnableToFindProcess(U32OrString::String(name.to_string())))?;
            if f.trim() == name {
                let pid = file.file_name().to_str().unwrap().parse::<u32>().unwrap();
                return Self::new(pid);
            }
        }
        Err(ProcessError::UnableToFindProcess(U32OrString::String(name.to_string())))
    }
